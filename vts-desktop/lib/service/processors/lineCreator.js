...
module.exports.process = async function(request, processor)
{
    ...
    for (let idx = 0; idx < processor.inputNodes.features.length; idx++)
    {
        ...
        for (let i = 0; i < files.length; i++)
        {
            ...
            let pointsFC = turf.explode(feature);
            let points = [];
            pointsFC.features.forEach(feat =>
            {
                points.push(feat.geometry);
            });

            let lineString = turf.lineString(points, feature.properties);
            ...
            let data = JSON.stringify(lineString);
