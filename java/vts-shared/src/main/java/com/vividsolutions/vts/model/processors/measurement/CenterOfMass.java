...
public class CenterOfMass extends FeatureProcessor
{
    private static Log logger = LogFactory.getLog(CenterOfMass.class);
    
    private static final long serialVersionUID = 3583721664593384362L;

    public CenterOfMass()
    {
        super();
        this.setType(Processor.Type.CENTER_OF_MASS.toString());
    }
    ...
    public boolean process() throws Exception
    {
        ...
        {   
            ...
            if(getFeatures().size() > 0) calculateCenterOfMass();
    ...
    private void calculateCenterOfMass() throws ParseException, JsonParseException, JsonMappingException, IOException
    {
        logger.debug(" >> calculateCenterOfMass()");
        ...
        if(getFeatures() != null && getFeatures().size() > 0)
        {
            getFeatures().parallelStream().forEach((featureId) -> 
            {
                try
                {
                    ...
                    Point centroid = feature.getAsGeometryObject().getCentroid();
    ...
    {
        CenterOfMass copy = new CenterOfMass();
